 ...
 where ss_item_sk = i_item_sk
   and d_date in (select d_date
                  ...
                  where d_week_seq = (select d_week_seq 
                                      from date_dim
                                      where d_date = to_date('2000-02-12', 'YYYY-MM-DD')))
...
select * from (select  
   w_state
  ,i_item_id
  ,sum(case when (to_date(d_date) < to_date('2002-05-18', 'YYYY-MM-DD')) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (to_date('2002-05-18', 'YYYY-MM-DD') - 30)
                and (to_date('2002-05-18', 'YYYY-MM-DD') + 30) 
 group by
    w_state,i_item_id
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between to_date('2001-08-04', 'YYYY-MM-DD') 
                  and (to_date('2001-08-04', 'YYYY-MM-DD') +  14)
