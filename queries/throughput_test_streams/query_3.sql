-- start query 1 in stream 3 using template query89.tpl
select * from (select  *
from(
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (1998) and
        ((i_category in ('Books','Sports','Music') and
          i_class in ('cooking','camping','pop')
         )
      or (i_category in ('Home','Shoes','Women') and
          i_class in ('curtains/drapes','athletic','maternity') 
        ))
...
-- start query 2 in stream 3 using template query5.tpl
with ssr as
 (select s_store_id,
 ...
 where date_sk = d_date_sk
       and d_date between to_date('2002-08-12', 'YYYY-MM-DD') 
                  and (to_date('2002-08-12', 'YYYY-MM-DD') +  14)
...
-- start query 3 in stream 3 using template query52.tpl
...
-- start query 4 in stream 3 using template query62.tpl
select * from (select  
...
-- start query 5 in stream 3 using template query53.tpl
...
-- start query 6 in stream 3 using template query7.tpl
...
-- start query 7 in stream 3 using template query39.tpl
with inv as
...
-- start query 8 in stream 3 using template query80.tpl
with ssr as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between to_date('1999-08-10', 'YYYY-MM-DD') 
                  and (to_date('1999-08-10', 'YYYY-MM-DD') + 30)
...
-- start query 10 in stream 3 using template query72.tpl
...
-- start query 11 in stream 3 using template query18.tpl
select * from (select  i_item_id,
        ca_country,
        ca_state, 
        ca_county,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (12,7,4,8,9,6) and
       d_year = 1998 and
       ca_state in ('KY','TN','AR'
                   ,'WI','GA','IA','SD')
...
-- start query 12 in stream 3 using template query56.tpl
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('linen','dark','indian'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2002
 and     d_moy                   = 2
...
-- start query 13 in stream 3 using template query13.tpl
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('TX', 'WI', 'CO')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('OK', 'KY', 'TN')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('NC', 'MS', 'VA')
  and ss_net_profit between 50 and 250  
     ))
;

-- end query 13 in stream 3 using template query13.tpl
-- start query 14 in stream 3 using template query69.tpl
select * from (select  
  cd_gender,
  cd_marital_status,
  cd_education_status,
...
-- start query 15 in stream 3 using template query23.tpl
...
-- start query 16 in stream 3 using template query19.tpl
...
-- start query 17 in stream 3 using template query74.tpl
with year_total as (
...
-- start query 18 in stream 3 using template query30.tpl
...
select * from (select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Sunnyside'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  3460
   and ib_upper_bound   <=  3460 + 50000
...
-- start query 20 in stream 3 using template query96.tpl
...
-- start query 21 in stream 3 using template query14.tpl
with  cross_items as
...
-- start query 22 in stream 3 using template query10.tpl
select * from (select  
  cd_gender,
  cd_marital_status,
  cd_education_status,
...
-- end query 22 in stream 3 using template query10.tpl
-- start query 23 in stream 3 using template query86.tpl
select * from (select   
...
-- start query 24 in stream 3 using template query94.tpl
select * from (select  
...
-- start query 26 in stream 3 using template query87.tpl
select count(*) 
...
-- start query 27 in stream 3 using template query58.tpl
with ss_items as
 ...
 where ss_item_sk = i_item_sk
   and d_date in (select d_date
                  ...
                  where d_week_seq = (select d_week_seq 
                                      from date_dim
                                      where d_date = to_date('1999-05-11', 'YYYY-MM-DD')))
   and ss_sold_date_sk   = d_date_sk
 group by i_item_id),
 cs_items as
 (select i_item_id item_id
...
-- start query 28 in stream 3 using template query36.tpl
select * from (select  
...
-- start query 29 in stream 3 using template query37.tpl
select * from (select  i_item_id
       ,i_item_desc
       ,i_current_price
 ...
 and d_date between to_date('1999-05-26', 'YYYY-MM-DD') and (to_date('1999-05-26', 'YYYY-MM-DD') +  60)
 and i_manufact_id in (888,745,693,800)
...
-- start query 30 in stream 3 using template query4.tpl
with year_total as (
 select c_customer_id customer_id
       ,c_first_name customer_first_name
       ,c_last_name customer_last_name
...
-- start query 31 in stream 3 using template query38.tpl
...
-- start query 32 in stream 3 using template query66.tpl
select * from (select   
         w_warehouse_name
 	,w_warehouse_sq_ft
 	,w_city
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 2000
 	and t_time between 22978 and 22978+28800 
 	and sm_carrier in ('UPS','DIAMOND')
     group by 
        w_warehouse_name
 	,w_warehouse_sq_ft
 	,w_city
...
-- start query 33 in stream 3 using template query78.tpl
with ws as
...
-- start query 34 in stream 3 using template query43.tpl
...
-- start query 35 in stream 3 using template query22.tpl
...
-- start query 36 in stream 3 using template query21.tpl
select * from (select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (to_date(d_date) < to_date('2000-04-16', 'YYYY-MM-DD'))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and inv_warehouse_sk   = w_warehouse_sk
     and inv_date_sk    = d_date_sk
     and d_date between (to_date('2000-04-16', 'YYYY-MM-DD') - 30)
                    and (to_date('2000-04-16', 'YYYY-MM-DD') + 30)
...
-- start query 37 in stream 3 using template query97.tpl
with ssci as (
...
-- start query 38 in stream 3 using template query47.tpl
with v1 as(
...
-- start query 39 in stream 3 using template query29.tpl
select * from (select   
     i_item_id
    ,i_item_desc
    ,s_store_id
...
-- start query 40 in stream 3 using template query3.tpl
select * from (select  dt.d_year 
       ,item.i_brand_id brand_id 
       ,item.i_brand brand
...
-- start query 41 in stream 3 using template query76.tpl
...
select * from (select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between to_date('2001-05-12', 'YYYY-MM-DD') and (to_date('2001-05-12', 'YYYY-MM-DD') +  60)
 and i_manufact_id in (543,730,134,242)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
-- start query 43 in stream 3 using template query46.tpl
select * from (select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and store.s_city in ('Fairview','Midway','Midway','Fairview','Fairview') 
...
-- start query 45 in stream 3 using template query59.tpl
with wss as 
 (select d_week_seq,
        ss_store_sk,
        sum(case when (d_day_name='Sunday') then ss_sales_price else null end) sun_sales,
 ...
 (select s_store_name s_store_name1,wss.d_week_seq d_week_seq1
  ...
  where d.d_week_seq = wss.d_week_seq and
        ss_store_sk = s_store_sk and 
        d_month_seq between 1208 and 1208 + 11) y,
 (select s_store_name s_store_name2,wss.d_week_seq d_week_seq2
  ...
  where d.d_week_seq = wss.d_week_seq and
        ss_store_sk = s_store_sk and 
        d_month_seq between 1208+ 12 and 1208 + 23) x
...
-- start query 46 in stream 3 using template query40.tpl
select * from (select  
   w_state
  ,i_item_id
  ,sum(case when (to_date(d_date) < to_date('1999-06-03', 'YYYY-MM-DD')) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (to_date('1999-06-03', 'YYYY-MM-DD') - 30)
                and (to_date('1999-06-03', 'YYYY-MM-DD') + 30) 
 group by
    w_state,i_item_id
...
-- start query 48 in stream 3 using template query16.tpl
select * from (select  
...
-- start query 49 in stream 3 using template query27.tpl
select * from (select  i_item_id,
...
-- start query 50 in stream 3 using template query54.tpl
...
-- start query 51 in stream 3 using template query90.tpl
...
i_manufact_id = 595
and i_item_sk = ws_item_sk 
and d_date between to_date('2002-03-13', 'YYYY-MM-DD') and 
        (to_date('2002-03-13', 'YYYY-MM-DD') + 90)
...
-- start query 53 in stream 3 using template query31.tpl
with ss as
...
-- start query 54 in stream 3 using template query42.tpl
select * from (select  dt.d_year
 	,item.i_category_id
 	,item.i_category
...
-- start query 55 in stream 3 using template query26.tpl
select * from (select  i_item_id, 
...
-- start query 56 in stream 3 using template query34.tpl
select c_last_name
       ,c_first_name
       ,c_salutation
       ,c_preferred_cust_flag
...
-- start query 57 in stream 3 using template query68.tpl
select * from (select  c_last_name
       ,c_first_name
       ,ca_city
       ,bought_city
...
-- start query 58 in stream 3 using template query2.tpl
with wscs as
 (select sold_date_sk
        ,sales_price
...
-- start query 59 in stream 3 using template query44.tpl
...
-- start query 60 in stream 3 using template query81.tpl
with customer_total_return as
...
-- start query 61 in stream 3 using template query67.tpl
select * from (select  *
...
-- start query 62 in stream 3 using template query99.tpl
select * from (select  
   substr(w_warehouse_name,1,20)
  ,sm_type
  ,cc_name
...
-- start query 63 in stream 3 using template query6.tpl
...
-- start query 64 in stream 3 using template query83.tpl
with sr_items as
 (select i_item_id item_id,
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (to_date('1999-06-05', 'YYYY-MM-DD'),to_date('1999-10-03', 'YYYY-MM-DD'),to_date('1999-11-18', 'YYYY-MM-DD'))))
...
-- start query 65 in stream 3 using template query1.tpl
with customer_total_return as
...
-- start query 66 in stream 3 using template query60.tpl
with ss as (
 select
...
-- start query 67 in stream 3 using template query33.tpl
with ss as (
 select
...
-- start query 68 in stream 3 using template query11.tpl
with year_total as (
 select c_customer_id customer_id
       ,c_first_name customer_first_name
       ,c_last_name customer_last_name
...
from (select avg(ss_list_price) B1_LP
      ...
      where ss_quantity between 0 and 5
        and (ss_list_price between 18 and 18+10 
             or ss_coupon_amt between 1705 and 1705+1000
     ...
     (select avg(ss_list_price) B2_LP
      ...
      where ss_quantity between 6 and 10
        and (ss_list_price between 185 and 185+10
          or ss_coupon_amt between 17091 and 17091+1000
     ...
     (select avg(ss_list_price) B3_LP
      ...
      where ss_quantity between 11 and 15
        and (ss_list_price between 6 and 6+10
          or ss_coupon_amt between 9594 and 9594+1000
     ...
     (select avg(ss_list_price) B4_LP
      ...
      where ss_quantity between 16 and 20
        and (ss_list_price between 138 and 138+10
          or ss_coupon_amt between 2078 and 2078+1000
...
-- start query 70 in stream 3 using template query95.tpl
with ws_wh as
...
from
  ...
  ,web_site
where
    d_date between to_date('2000-5-01', 'YYYY-MM-DD') and 
           (to_date('2000-5-01', 'YYYY-MM-DD') + 60)
and ws1.ws_ship_date_sk = d_date_sk
and ws1.ws_ship_addr_sk = ca_address_sk
and ca_state = 'AL'
and ws1.ws_web_site_sk = web_site_sk
...
-- start query 71 in stream 3 using template query12.tpl
select * from (select  i_item_id
      ,i_item_desc 
      ,i_category 
      ,i_class 
...
where 
	ws_item_sk = i_item_sk 
  	and i_category in ('Men', 'Sports', 'Electronics')
  	and ws_sold_date_sk = d_date_sk
	and d_date between to_date('2002-06-04', 'YYYY-MM-DD') 
				and (to_date('2002-06-04', 'YYYY-MM-DD') + 30)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
-- start query 72 in stream 3 using template query64.tpl
with cs_ui as
 (select cs_item_sk
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('violet','bisque','black','khaki','pale','ghost') and
         i_current_price between 66 and 66 + 10 and
         i_current_price between 66 + 1 and 66 + 15
...
-- start query 73 in stream 3 using template query15.tpl
...
-- start query 74 in stream 3 using template query25.tpl
select * from (select  
 i_item_id
 ,i_item_desc
 ,s_store_id
...
-- start query 75 in stream 3 using template query93.tpl
...
-- start query 78 in stream 3 using template query71.tpl
...
-- start query 79 in stream 3 using template query57.tpl
with v1 as(
 select i_category, i_brand,
        cc_name,
        d_year, d_moy,
...
i_manufact_id = 212
and i_item_sk = cs_item_sk 
and d_date between to_date('2000-03-03', 'YYYY-MM-DD') and 
        (to_date('2000-03-03', 'YYYY-MM-DD') + 90)
...
-- start query 81 in stream 3 using template query61.tpl
...
-- start query 82 in stream 3 using template query85.tpl
...
-- start query 83 in stream 3 using template query73.tpl
select c_last_name
       ,c_first_name
       ,c_salutation
       ,c_preferred_cust_flag 
...
where 
	ss_item_sk = i_item_sk 
  	...
  	and ss_sold_date_sk = d_date_sk
	and d_date between to_date('2000-02-24', 'YYYY-MM-DD') 
				and (to_date('2000-02-24', 'YYYY-MM-DD') + 30)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
-- start query 85 in stream 3 using template query77.tpl
with ss as
 (select s_store_sk,
         sum(ss_ext_sales_price) as sales,
 ...
 from store_sales,
      ...
      store
 where ss_sold_date_sk = d_date_sk
       and d_date between to_date('2000-08-05', 'YYYY-MM-DD') 
                  and (to_date('2000-08-05', 'YYYY-MM-DD') +  30) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
-- start query 86 in stream 3 using template query45.tpl
select * from (select  ca_zip, ca_state, sum(ws_sales_price)
...
-- start query 87 in stream 3 using template query20.tpl
select * from (select  i_item_id
       ,i_item_desc 
       ,i_category 
       ,i_class 
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between to_date('1998-04-05', 'YYYY-MM-DD') 
 				and (to_date('1998-04-05', 'YYYY-MM-DD') + 30)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
-- start query 88 in stream 3 using template query50.tpl
select * from (select  
   s_store_name
  ,s_company_id
  ,s_street_number
...
-- start query 89 in stream 3 using template query70.tpl
select * from (select  
...
-- start query 90 in stream 3 using template query75.tpl
WITH all_sales AS (
 SELECT d_year
       ,i_brand_id
       ,i_class_id
...
-- start query 91 in stream 3 using template query91.tpl
select  
...
-- start query 92 in stream 3 using template query17.tpl
select * from (select  i_item_id
       ,i_item_desc
       ,s_state
...
-- start query 93 in stream 3 using template query24.tpl
with ssales as
(select c_last_name
      ,c_first_name
      ,s_store_name
...
-- start query 94 in stream 3 using template query48.tpl
...
-- start query 95 in stream 3 using template query51.tpl
WITH web_v1 as (
select
...
-- start query 96 in stream 3 using template query79.tpl
select * from (select 
...
-- start query 97 in stream 3 using template query35.tpl
select * from (select   
  ca_state,
  cd_gender,
  cd_marital_status,
