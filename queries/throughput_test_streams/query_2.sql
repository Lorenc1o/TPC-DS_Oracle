-- start query 1 in stream 2 using template query56.tpl
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('khaki','deep','forest'))
...
-- start query 2 in stream 2 using template query98.tpl
select i_item_id
      ,i_item_desc 
      ,i_category 
      ,i_class 
...
where 
	ss_item_sk = i_item_sk 
  	and i_category in ('Women', 'Children', 'Electronics')
  	and ss_sold_date_sk = d_date_sk
	and d_date between to_date('1999-05-25', 'YYYY-MM-DD') 
				and (to_date('1999-05-25', 'YYYY-MM-DD') + 30)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
-- start query 3 in stream 2 using template query59.tpl
with wss as 
 (select d_week_seq,
        ss_store_sk,
...
-- start query 4 in stream 2 using template query24.tpl
with ssales as
(select c_last_name
      ,c_first_name
      ,s_store_name
...
-- start query 5 in stream 2 using template query88.tpl
select  *
from
...
-- start query 6 in stream 2 using template query2.tpl
with wscs as
 (select sold_date_sk
        ,sales_price
...
-- start query 7 in stream 2 using template query5.tpl
with ssr as
 (select s_store_id,
 ...
 where date_sk = d_date_sk
       and d_date between to_date('2001-08-11', 'YYYY-MM-DD') 
                  and (to_date('2001-08-11', 'YYYY-MM-DD') +  14)
...
-- end query 7 in stream 2 using template query5.tpl
-- start query 8 in stream 2 using template query6.tpl
...
-- start query 9 in stream 2 using template query27.tpl
...
-- end query 9 in stream 2 using template query27.tpl
-- start query 10 in stream 2 using template query87.tpl
select count(*) 
...
-- start query 11 in stream 2 using template query90.tpl
...
-- start query 12 in stream 2 using template query83.tpl
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in (to_date('1998-07-21', 'YYYY-MM-DD'),to_date('1998-09-26', 'YYYY-MM-DD'),to_date('1998-11-21', 'YYYY-MM-DD'))))
...
-- start query 13 in stream 2 using template query91.tpl
select  
...
from (select avg(ss_list_price) B1_LP
      ...
      where ss_quantity between 0 and 5
        and (ss_list_price between 3 and 3+10 
             or ss_coupon_amt between 9546 and 9546+1000
     ...
     (select avg(ss_list_price) B2_LP
      ...
      where ss_quantity between 6 and 10
        and (ss_list_price between 162 and 162+10
          or ss_coupon_amt between 6449 and 6449+1000
     ...
     (select avg(ss_list_price) B3_LP
      ...
      where ss_quantity between 11 and 15
        and (ss_list_price between 38 and 38+10
          or ss_coupon_amt between 13792 and 13792+1000
     ...
     (select avg(ss_list_price) B4_LP
      ...
      where ss_quantity between 16 and 20
        and (ss_list_price between 140 and 140+10
          or ss_coupon_amt between 14178 and 14178+1000
     ...
     (select avg(ss_list_price) B6_LP
      ...
      where ss_quantity between 26 and 30
        and (ss_list_price between 39 and 39+10
          or ss_coupon_amt between 1710 and 1710+1000
          or ss_wholesale_cost between 46 and 46+20)) B6
 ) where rownum <= 100;

-- end query 14 in stream 2 using template query28.tpl
-- start query 15 in stream 2 using template query68.tpl
...
-- start query 17 in stream 2 using template query76.tpl
...
-- start query 19 in stream 2 using template query80.tpl
with ssr as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between to_date('2000-08-12', 'YYYY-MM-DD') 
                  and (to_date('2000-08-12', 'YYYY-MM-DD') + 30)
       and ss_store_sk = s_store_sk
       and ss_item_sk = i_item_sk
...
-- end query 19 in stream 2 using template query80.tpl
-- start query 20 in stream 2 using template query1.tpl
...
-- start query 21 in stream 2 using template query69.tpl
select * from (select  
  cd_gender,
  cd_marital_status,
  cd_education_status,
...
-- start query 22 in stream 2 using template query26.tpl
select * from (select  i_item_id, 
...
-- start query 23 in stream 2 using template query11.tpl
with year_total as (
...
-- start query 24 in stream 2 using template query17.tpl
select * from (select  i_item_id
       ,i_item_desc
       ,s_state
...
-- start query 25 in stream 2 using template query63.tpl
select * from (select  * 
...
-- start query 26 in stream 2 using template query77.tpl
with ss as
 (select s_store_sk,
         sum(ss_ext_sales_price) as sales,
 ...
 from store_sales,
      ...
      store
 where ss_sold_date_sk = d_date_sk
       and d_date between to_date('2002-08-17', 'YYYY-MM-DD') 
                  and (to_date('2002-08-17', 'YYYY-MM-DD') +  30) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
-- end query 26 in stream 2 using template query77.tpl
-- start query 27 in stream 2 using template query19.tpl
...
-- start query 28 in stream 2 using template query21.tpl
select * from (select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (to_date(d_date) < to_date('1999-05-04', 'YYYY-MM-DD'))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (to_date('1999-05-04', 'YYYY-MM-DD') - 30)
                    and (to_date('1999-05-04', 'YYYY-MM-DD') + 30)
...
-- start query 29 in stream 2 using template query31.tpl
with ss as
...
-- start query 30 in stream 2 using template query93.tpl
...
-- start query 31 in stream 2 using template query54.tpl
...
-- start query 32 in stream 2 using template query39.tpl
with inv as
...
-- start query 33 in stream 2 using template query10.tpl
select * from (select  
  cd_gender,
  cd_marital_status,
  cd_education_status,
 ...
 where
  c.c_current_addr_sk = ca.ca_address_sk and
  ca_county in ('Quay County','Bourbon County','Milam County','Mineral County','Christian County') and
  cd_demo_sk = c.c_current_cdemo_sk and 
  exists (select *
          from store_sales,date_dim
          where c.c_customer_sk = ss_customer_sk and
...
-- start query 34 in stream 2 using template query15.tpl
...
-- start query 35 in stream 2 using template query55.tpl
...
-- start query 36 in stream 2 using template query14.tpl
with  cross_items as
...
-- end query 36 in stream 2 using template query14.tpl
-- start query 37 in stream 2 using template query38.tpl
...
-- start query 38 in stream 2 using template query42.tpl
...
-- start query 39 in stream 2 using template query53.tpl
...
-- start query 40 in stream 2 using template query45.tpl
...
-- start query 41 in stream 2 using template query99.tpl
select * from (select  
...
-- start query 42 in stream 2 using template query67.tpl
select * from (select  *
...
-- end query 42 in stream 2 using template query67.tpl
-- start query 43 in stream 2 using template query23.tpl
...
-- start query 44 in stream 2 using template query62.tpl
select * from (select  
   substr(w_warehouse_name,1,20)
  ,sm_type
  ,web_name
...
-- start query 45 in stream 2 using template query30.tpl
with customer_total_return as
...
-- start query 46 in stream 2 using template query86.tpl
select * from (select   
...
-- end query 46 in stream 2 using template query86.tpl
-- start query 47 in stream 2 using template query82.tpl
select * from (select  i_item_id
       ,i_item_desc
       ,i_current_price
 ...
 and d_date between to_date('2002-01-20', 'YYYY-MM-DD') and (to_date('2002-01-20', 'YYYY-MM-DD') +  60)
 and i_manufact_id in (761,770,591,91)
...
-- start query 48 in stream 2 using template query25.tpl
select * from (select  
 i_item_id
 ,i_item_desc
 ,s_store_id
...
-- start query 49 in stream 2 using template query16.tpl
select * from (select  
...
-- start query 50 in stream 2 using template query81.tpl
with customer_total_return as
...
-- start query 51 in stream 2 using template query40.tpl
select * from (select  
   w_state
  ,i_item_id
  ,sum(case when (to_date(d_date) < to_date('2001-02-24', 'YYYY-MM-DD')) 
 ...
 and cs_warehouse_sk    = w_warehouse_sk 
 and cs_sold_date_sk    = d_date_sk
 and d_date between (to_date('2001-02-24', 'YYYY-MM-DD') - 30)
                and (to_date('2001-02-24', 'YYYY-MM-DD') + 30) 
 group by
    w_state,i_item_id
...
-- start query 52 in stream 2 using template query44.tpl
...
-- start query 53 in stream 2 using template query50.tpl
select * from (select  
   s_store_name
  ,s_company_id
  ,s_street_number
...
-- start query 54 in stream 2 using template query61.tpl
...
-- start query 55 in stream 2 using template query85.tpl
...
-- start query 56 in stream 2 using template query73.tpl
select c_last_name
       ,c_first_name
       ,c_salutation
       ,c_preferred_cust_flag 
...
-- start query 57 in stream 2 using template query95.tpl
with ws_wh as
...
-- start query 58 in stream 2 using template query84.tpl
select * from (select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Marion'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  41702
   and ib_upper_bound   <=  41702 + 50000
...
-- start query 59 in stream 2 using template query4.tpl
with year_total as (
 select c_customer_id customer_id
       ,c_first_name customer_first_name
       ,c_last_name customer_last_name
...
select * from (select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between to_date('2002-03-22', 'YYYY-MM-DD') and (to_date('2002-03-22', 'YYYY-MM-DD') +  60)
 and i_manufact_id in (949,857,827,944)
 and inv_quantity_on_hand between 100 and 500
 and cs_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
-- start query 61 in stream 2 using template query35.tpl
select * from (select   
  ca_state,
  cd_gender,
  cd_marital_status,
...
-- start query 62 in stream 2 using template query94.tpl
select * from (select  
   count(distinct ws_order_number) as "order count"
  ,sum(ws_ext_ship_cost) as "total shipping cost"
  ,sum(ws_net_profit) as "total net profit"
...
-- start query 63 in stream 2 using template query58.tpl
with ss_items as
 (select i_item_id item_id
 ...
 where ss_item_sk = i_item_sk
   and d_date in (select d_date
                  ...
                  where d_week_seq = (select d_week_seq 
                                      from date_dim
                                      where d_date = to_date('2002-07-07', 'YYYY-MM-DD')))
   and ss_sold_date_sk   = d_date_sk
 group by i_item_id),
 cs_items as
 (select i_item_id item_id
...
-- start query 64 in stream 2 using template query96.tpl
...
-- start query 65 in stream 2 using template query12.tpl
select * from (select  i_item_id
      ,i_item_desc 
      ,i_category 
      ,i_class 
...
where 
	ws_item_sk = i_item_sk 
  	and i_category in ('Men', 'Jewelry', 'Electronics')
  	and ws_sold_date_sk = d_date_sk
	and d_date between to_date('2001-06-05', 'YYYY-MM-DD') 
				and (to_date('2001-06-05', 'YYYY-MM-DD') + 30)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
-- start query 66 in stream 2 using template query29.tpl
select * from (select   
     i_item_id
    ,i_item_desc
    ,s_store_id
...
-- start query 67 in stream 2 using template query22.tpl
...
-- end query 67 in stream 2 using template query22.tpl
-- start query 68 in stream 2 using template query51.tpl
WITH web_v1 as (
select
...
-- start query 69 in stream 2 using template query36.tpl
select * from (select  
...
-- start query 70 in stream 2 using template query43.tpl
...
-- start query 71 in stream 2 using template query64.tpl
with cs_ui as
 (select cs_item_sk
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('snow','salmon','bisque','tomato','cornsilk','cream') and
...
select * from (select  i_item_id
 ...
 from	catalog_sales
     ...
     ,date_dim
 where cs_item_sk = i_item_sk 
   and i_category in ('Children', 'Women', 'Sports')
   and cs_sold_date_sk = d_date_sk
 and d_date between to_date('2001-04-06', 'YYYY-MM-DD') 
...
-- start query 73 in stream 2 using template query57.tpl
with v1 as(
...
-- start query 74 in stream 2 using template query9.tpl
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 5563
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 43790
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 2348
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 42042
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 32190
            then (select avg(ss_ext_tax)
                  from store_sales
...
-- start query 75 in stream 2 using template query52.tpl
select * from (select  dt.d_year
...
-- start query 76 in stream 2 using template query49.tpl
...
-- start query 77 in stream 2 using template query71.tpl
...
-- start query 78 in stream 2 using template query72.tpl
...
-- start query 79 in stream 2 using template query70.tpl
select * from (select  
...
-- end query 79 in stream 2 using template query70.tpl
-- start query 80 in stream 2 using template query7.tpl
select * from (select  i_item_id, 
        avg(ss_quantity) agg1,
        avg(ss_list_price) agg2,
        avg(ss_coupon_amt) agg3,
...
-- start query 81 in stream 2 using template query97.tpl
with ssci as (
...
-- start query 82 in stream 2 using template query33.tpl
with ss as (
 select
...
-- start query 83 in stream 2 using template query79.tpl
select * from (select 
...
-- start query 84 in stream 2 using template query32.tpl
...
i_manufact_id = 572
and i_item_sk = cs_item_sk 
and d_date between to_date('1998-01-17', 'YYYY-MM-DD') and 
        (to_date('1998-01-17', 'YYYY-MM-DD') + 90)
...
-- start query 85 in stream 2 using template query78.tpl
with ws as
...
-- start query 86 in stream 2 using template query18.tpl
select * from (select  i_item_id,
        ca_country,
        ca_state, 
        ca_county,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (8,7,9,5,10,4) and
       d_year = 2002 and
       ca_state in ('NY','NJ','GA'
                   ,'KY','MO','TX','OH')
...
-- end query 86 in stream 2 using template query18.tpl
-- start query 87 in stream 2 using template query65.tpl
select * from (select 
	s_store_name,
	i_item_desc,
	sc.revenue,
...
-- start query 88 in stream 2 using template query60.tpl
with ss as (
 select
...
-- start query 89 in stream 2 using template query34.tpl
select c_last_name
       ,c_first_name
       ,c_salutation
       ,c_preferred_cust_flag
...
-- start query 91 in stream 2 using template query13.tpl
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('TX', 'MA', 'MO')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('IN', 'WV', 'GA')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('OH', 'LA', 'CA')
...
-- start query 93 in stream 2 using template query92.tpl
select * from (select  
...
i_manufact_id = 21
and i_item_sk = ws_item_sk 
and d_date between to_date('1998-03-24', 'YYYY-MM-DD') and 
        (to_date('1998-03-24', 'YYYY-MM-DD') + 90)
...
-- start query 94 in stream 2 using template query74.tpl
with year_total as (
 select c_customer_id customer_id
       ,c_first_name customer_first_name
       ,c_last_name customer_last_name
...
-- start query 95 in stream 2 using template query46.tpl
select * from (select  c_last_name
       ,c_first_name
       ,ca_city
       ,bought_city
...
-- start query 96 in stream 2 using template query89.tpl
select * from (select  *
from(
...
where ss_item_sk = i_item_sk and
      ...
      ss_store_sk = s_store_sk and
      d_year in (2000) and
        ((i_category in ('Books','Music','Sports') and
          i_class in ('parenting','classical','basketball')
         )
      or (i_category in ('Electronics','Men','Shoes') and
          i_class in ('disk drives','accessories','mens') 
        ))
...
-- start query 97 in stream 2 using template query47.tpl
with v1 as(
 select i_category, i_brand,
        s_store_name, s_company_name,
        d_year, d_moy,
...
-- start query 98 in stream 2 using template query66.tpl
select * from (select   
         w_warehouse_name
 	,w_warehouse_sq_ft
 	,w_city
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1998
 	and t_time between 41169 and 41169+28800 
...
-- start query 99 in stream 2 using template query48.tpl
